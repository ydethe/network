...
# def post_person_data(db: Session, user_id: int, person_id:int, ) -> schemas.PersonDataModel:
#     ores: models.PersonData = db.query(models.PersonData).filter(models.PersonData.user_id == user_id).filter(models.PersonData.person_id == person_id).first()
#     res = schemas.PersonDataModel.fromORM(ores)
#     return res
